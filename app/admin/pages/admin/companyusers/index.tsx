...
const Companyusers = () => {
  ...
  const [{ companyusers, hasMore, count }] = usePaginatedQuery(getCompanyusers, {
    where: {
      ...query,
    },
  ...
  useMemo(async () => {
    ...
    await companyusers?.forEach((companyuser) => {
      data = [
        ...data,
        {
          ...companyuser,
  ...
  const columns = [
    {
      ...
      Cell: (props) => {
        return (
          <Link
            href={Routes.SingleCompanyuserAdminPage({
...
const AdminCompanyuser = ({ user }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  return (
...
AdminCompanyuser.authenticate = true

export default AdminCompanyuser
