...
const Jobusers = () => {
  ...
  const [{ jobusers, hasMore, count }] = usePaginatedQuery(getJobusers, {
    where: {
      ...query,
    },
  ...
  useMemo(async () => {
    ...
    await jobusers.forEach((jobuser) => {
      data = [
        ...data,
        {
          ...jobuser,
  ...
  const columns = [
    {
      ...
      Cell: (props) => {
        return (
          <Link
            href={Routes.SingleJobuserAdminPage({
...
const AdminJobuser = ({ user }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  return (
...
AdminJobuser.authenticate = true

export default AdminJobuser
