...
const Defaultcalendars = () => {
  ...
  const [{ defaultcalendars, hasMore, count }] = usePaginatedQuery(getDefaultcalendars, {
    where: {
      ...query,
    },
  ...
  const columns = [
    {
      ...
      Cell: (props) => {
        return (
          <Link
            href={Routes.SingleDefaultcalendarAdminPage({
...
const AdminDefaultcalendar = ({ user }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  return (
    <AuthLayout title={`Admin | 1UpBlitz`} user={user}>
      <AdminLayout>
        <div className="bg-gray-200 w-full p-2 rounded text-gray-500">
          <p>Defaultcalendars</p>
        </div>
...
AdminDefaultcalendar.authenticate = true

export default AdminDefaultcalendar
