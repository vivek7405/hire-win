...
import { JobUserRole, TokenType, UserRole } from "@prisma/client"
...
export default api(
  api(async (req, res) => {
    ...
    const possibleToken = await db.token.findFirst({
      where: { hashedToken, type: TokenType.INVITE_TO_JOB },
      include: {
        user: {
          select: {
            id: true,
    ...
    */
      res.redirect(
        `${process.env.NEXT_PUBLIC_APP_URL}/auth/login?next=/api/invitations/job/accept/&token=${req.query.token}`
      )
    } else {
